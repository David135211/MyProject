...
BOOL CSwitch_2pos::DrawElemnt( QPainter *pPainter )
{
    if( !pPainter )
    {
        Q_ASSERT( FALSE );
...
BOOL CSwitch_2pos::Create( QRect rc )
{
    m_rcDraw = rc ;
    return TRUE ;
}

void CSwitch_2pos::CreateMenu()
{
...
void CSwitch_2pos::DrawVertical( QPainter *pPainter )
{
...
void CSwitch_2pos::DrawVerticalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_2pos::DrawVerticalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_2pos::DrawHorizontal( QPainter *pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_2pos::DrawHorizontalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_2pos::DrawHorizontalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_2pos::DrawIsChoice( QPainter * pPainter )
{
    if( m_byIsChoice )
    {
        pPainter->save();
...
BOOL CSwitch_2pos::CompareAttributeData( )
{
...
BOOL CSwitch_2pos::ProcessMouseDown( QMouseEvent * pEvent )
{
    m_byIsChoice = TRUE;
...
void CSwitch_2pos::slots_QMenuAction(QAction * action)
{
    if( action == NULL )
        return;
...
void CSwitch_2pos::SetAction(BYTE Action)
{
//    WORD wBusNoDst = -1;
//    WORD wDevAddrDst = -1;
//    WORD wPntDst = -1;
...
BOOL CSwitch_2pos::IsChangeColor( )
{
...
BOOL CSwitch_2pos::SetChangeNodeColor(WORD ChangeNode, BYTE PowerState)
{
    m_byLeftTopPower = POWER_OFF;
    m_byRightBottomPower = POWER_OFF;
