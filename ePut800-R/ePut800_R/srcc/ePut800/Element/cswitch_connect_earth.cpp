...
CSwitch_connect_earth::CSwitch_connect_earth()
{
...
BOOL CSwitch_connect_earth::DrawElemnt( QPainter *pPainter )
{
    if( !pPainter )
    {
        Q_ASSERT( FALSE );
...
BOOL CSwitch_connect_earth::InitData( )
{
...
BOOL CSwitch_connect_earth::Create( QRect rc )
{
    m_rcDraw = rc ;
    return TRUE ;
}

void CSwitch_connect_earth::CreateMenu()
{
...
void CSwitch_connect_earth::InitMenu( )
{
    char BusAddrPnt[15] = "";
...
void CSwitch_connect_earth::DrawVertical( QPainter *pPainter )
{
...
void CSwitch_connect_earth::DrawVerticalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_connect_earth::DrawVerticalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
   ...
   else if( BREAKER_CLOSE == m_byOpen )
   {
    ...
    wXMoveTo = wXMoveTo;
    wYMoveTo = wYMoveTo;
    wXLineTo = m_rcDraw.x()+m_rcDraw.width()/4 ;
    wYLineTo = wYMoveTo;
    pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );
...
void CSwitch_connect_earth::DrawHorizontal( QPainter *pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_connect_earth::DrawHorizontalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_connect_earth::DrawHorizontalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CSwitch_connect_earth::DrawIsChoice( QPainter * pPainter )
{
    if( m_byIsChoice )
    {
        pPainter->save();
...
BOOL CSwitch_connect_earth::CompareAttributeData( )
{
...
BOOL CSwitch_connect_earth::ProcessMouseDown( QMouseEvent * pEvent )
{
    m_byIsChoice = TRUE;
...
void CSwitch_connect_earth::slots_QMenuAction(QAction * action)
{
    if( action == NULL )
        return;
...
void CSwitch_connect_earth::SetAction(BYTE Action)
{
//    WORD wBusNoDst = -1;
//    WORD wDevAddrDst = -1;
//    WORD wPntDst = -1;
...
BOOL CSwitch_connect_earth::IsChangeColor( )
{
...
BOOL CSwitch_connect_earth::SetChangeNodeColor(WORD ChangeNode, BYTE PowerState)
{
    m_byLeftTopPower = POWER_OFF;
    m_byRightBottomPower = POWER_OFF;
