...
CLoad_breaker_2pos::CLoad_breaker_2pos()
{
...
BOOL CLoad_breaker_2pos::DrawElemnt( QPainter *pPainter )
{
    if( !pPainter )
    {
        Q_ASSERT( FALSE );
...
BOOL CLoad_breaker_2pos::InitData( )
{
...
BOOL CLoad_breaker_2pos::Create( QRect rc )
{
    m_rcDraw = rc ;
    return TRUE ;
}

void CLoad_breaker_2pos::CreateMenu()
{
...
void CLoad_breaker_2pos::InitMenu( )
{
    char BusAddrPnt[15] = "";
...
void CLoad_breaker_2pos::DrawVertical( QPainter *pPainter )
{
...
void CLoad_breaker_2pos::DrawVerticalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CLoad_breaker_2pos::DrawVerticalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CLoad_breaker_2pos::DrawHorizontal( QPainter *pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CLoad_breaker_2pos::DrawHorizontalTop( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CLoad_breaker_2pos::DrawHorizontalBottom( QPainter * pPainter )
{
    if( !pPainter || m_rcDraw.isNull() )
    {
        Q_ASSERT( FALSE ) ;
...
void CLoad_breaker_2pos::DrawIsChoice( QPainter * pPainter )
{
    if( m_byIsChoice )
    {
        pPainter->save();
...
BOOL CLoad_breaker_2pos::CompareAttributeData( )
{
...
BOOL CLoad_breaker_2pos::ProcessMouseDown( QMouseEvent * pEvent )
{
    m_byIsChoice = TRUE;
...
void CLoad_breaker_2pos::slots_QMenuAction(QAction * action)
{
    if( action == NULL )
        return;
...
void CLoad_breaker_2pos::SetAction(BYTE Action)
{
//    WORD wBusNoDst = -1;
//    WORD wDevAddrDst = -1;
//    WORD wPntDst = -1;
...
BOOL CLoad_breaker_2pos::IsChangeColor( )
{
...
BOOL CLoad_breaker_2pos::SetChangeNodeColor(WORD ChangeNode, BYTE PowerState)
{
    m_byLeftTopPower = POWER_OFF;
    m_byRightBottomPower = POWER_OFF;
