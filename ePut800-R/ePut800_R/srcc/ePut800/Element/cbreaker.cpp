...
{/*{{{*/
	...
	CreateMenu();
	// CreateYkDiallog( TWOPOS , m_Menu );
}/*}}}*/

BOOL CBreaker::DrawElemnt( QPainter *pPainter )
{/*{{{*/
	if( !pPainter)
	{
		Q_ASSERT( FALSE ) ;
...
{/*{{{*/
	...
	//m_YxTab->setWindowFlags(m_YxTab->windowFlags()&~Qt::WindowMaximizeButtonHint&~Qt::WindowMinimizeButtonHint | Qt::WindowStaysOnTopHint );
...
void CBreaker::DrawVertical( QPainter *pPainter )
{/*{{{*/
...
void CBreaker::DrawVerticalTop( QPainter * pPainter )
{/*{{{*/
	if( !pPainter || m_rcDraw.isNull() )
	{
		Q_ASSERT( FALSE ) ;
	...
	QPoint ptCenter = m_rcDraw.center() ;
	WORD cy = m_rcDraw.top();
	WORD wHight = m_rcDraw.height();
	WORD wWidth = m_rcDraw.width() ;

	//draw vertical line
	WORD wXMoveTo = ptCenter.x() ;
	WORD wYMoveTo = cy ;
	WORD wXLineTo = ptCenter.x() ;
	WORD wYLineTo = cy + wHight / 3 ;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );

	//draw Cross line
	//left begin right end
	wXMoveTo = m_rcDraw.left() + wWidth * 3  / 8 ;
	wYMoveTo = cy + wHight * 2 / 9;
	wXLineTo = m_rcDraw.left() + wWidth * 5 / 8 ;
	wYLineTo = cy + wHight * 4 / 9 ;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );

	//right begin left end
	wXMoveTo = m_rcDraw.left() + wWidth * 5 / 8 ;
	wYMoveTo = cy + wHight * 2 / 9;
	wXLineTo = m_rcDraw.left() + wWidth * 3 / 8 ;
	wYLineTo = cy + wHight * 4 / 9;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );
	pPainter->restore();
}/*}}}*/

void CBreaker::DrawVerticalBottom( QPainter * pPainter )
{/*{{{*/
	if( !pPainter || m_rcDraw.isNull() )
	{
		Q_ASSERT( FALSE ) ;
...
void CBreaker::DrawHorizontal( QPainter *pPainter )
{/*{{{*/
	if( !pPainter || m_rcDraw.isNull() )
	{
		Q_ASSERT( FALSE ) ;
...
void CBreaker::DrawHorizontalTop( QPainter * pPainter )
{/*{{{*/
	if( !pPainter || m_rcDraw.isNull() )
	{
		Q_ASSERT( FALSE ) ;
	...
	//draw vertical line
	WORD wXMoveTo = cy ;
	WORD wYMoveTo = ptCenter.y() ;
	WORD wXLineTo = cy + wWidth / 3 ;
	WORD wYLineTo = ptCenter.y() ;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );

	//draw Cross line
	//left begin right end
	wXMoveTo = cy + wWidth / 6 ;
	wYMoveTo = m_rcDraw.top() + wHight * 3  / 8 ;
	wXLineTo = cy + wWidth / 2 ;
	wYLineTo = m_rcDraw.top() + wHight * 5 / 8 ;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );
	...
	wXLineTo = cy + wWidth / 2 ;
	wYLineTo = m_rcDraw.top() + wHight * 3 / 8 ;
	pPainter->drawLine(QPoint(wXMoveTo , wYMoveTo ) , QPoint( wXLineTo , wYLineTo ) );
	pPainter->restore();
}/*}}}*/

void CBreaker::DrawHorizontalBottom( QPainter * pPainter )
{/*{{{*/
	if( !pPainter || m_rcDraw.isNull() )
	{
		Q_ASSERT( FALSE ) ;
...
void CBreaker::DrawIsChoice( QPainter * pPainter )
{/*{{{*/
	if( m_byIsChoice )
	{
		pPainter->save();
...
BOOL CBreaker::CompareAttributeData( )
{/*{{{*/
	...
	for( AttributeDataNo = 0; AttributeDataNo < AttributeDataSize; AttributeDataNo++ )
	{
		...
		case PIC_ATTRIBUTE_DATA_ELEMENT::YX_DTYPE:
			if( tmpData->byAttributeDataByte == 1 )
			{
				...
				//printf("P%d, %d, %d val = %d\n",tmpData->wBusNo,tmpData->wAddr,tmpData->wPntNo,byVal);
...
BOOL CBreaker::ProcessMouseDown( QMouseEvent * pEvent )
{/*{{{*/
	m_byIsChoice = TRUE;
	AfxGetWidget()->update(m_rcDraw);
	m_Menu->exec(pEvent->globalPos());
	//if( YkDiallog->isHidden() )
	m_byIsChoice = FALSE;
	AfxGetWidget()->update(m_rcDraw);
	return TRUE;
}/*}}}*/

void CBreaker::slots_QMenuAction(QAction * action)
{/*{{{*/
	if( action == NULL )
		return;
...
void CBreaker::SetAction(BYTE Action)
{/*{{{*/
	//WORD wBusNoDst = -1;
	//WORD wDevAddrDst = -1;
	//WORD wPntDst = -1;
	...
	case BREAKER_OPEN:
		...
		if( YkData[YkActionType].byAction == YK_OPEN )				//YkData:在InitYkData函数中被初始化!
			byYkSetState = TRUE;
		break;
	case BREAKER_CLOSE:
...
BOOL CBreaker::SetChangeNodeColor(WORD ChangeNode, BYTE PowerState)
{/*{{{*/
	m_byLeftTopPower = POWER_OFF;
	m_byRightBottomPower = POWER_OFF;
