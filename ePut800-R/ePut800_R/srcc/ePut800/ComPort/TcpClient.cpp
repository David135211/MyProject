...
BOOL CTcpClient::SetPreOpenPara( Property * pProperty )
{
    if( !pProperty )
    {
        Q_ASSERT( FALSE ) ;
    ...
    if( pProperty->m_ComType != Property::LAN_CLIENT )
    {
        Q_ASSERT( FALSE );
        return FALSE ;
    }

    LAN_CLIENT_Property * pLanProperty = ( LAN_CLIENT_Property *)pProperty ;
    m_uThePort = pLanProperty->wPortNo ;
    strcpy( m_szAttrib , pLanProperty->clientIp ) ;
