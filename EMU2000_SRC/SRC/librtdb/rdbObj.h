...
class CRTDBObj
{
	...
	public:
		...
		int CreateRTDBObj(char *szPrompt=NULL);
		void RTDBInit(void);
		...
		int GetPulseVal(WORD wStn, WORD wPnt, DWORD* pdwVal);
		int WritePulseVal(WORD wStn, WORD wPnt, DWORD dwVal, void *ts=NULL);
		/* Relay */
