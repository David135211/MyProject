...
CProtocol_Resume::CProtocol_Resume()
{
...
CProtocol_Resume::~CProtocol_Resume()
{
	...
	m_module.clear() ;
	printf( "Delete All CProtocol_Resume OK . \n" );
}



BOOL CProtocol_Resume::GetProtocolBuf( BYTE * buf , int &len , PBUSMSG pBusMsg ) 
{
	return TRUE;
}

BOOL CProtocol_Resume::ProcessProtocolBuf( BYTE * buf , int len ) 
{
	return FALSE ;
}

BOOL CProtocol_Resume::Init( BYTE byLineNo )
{
	//增加UPS 采集模块数据
	//通过总线号获取读取的装置文件路径
	m_byLineNo = byLineNo ;
...
BOOL CProtocol_Resume::GetDevData( )
{
	memset( m_sDevPath , 0 , sizeof( m_sDevPath ) ) ;
	sprintf( m_sDevPath , "%s/Resume/%s%02d.ini" , SYSDATAPATH , DEVNAME , m_byLineNo + 1 );
	CProfile profile( m_sDevPath ) ;
	return ProcessFileData( profile ) ;
}

BOOL CProtocol_Resume::ProcessFileData( CProfile &profile )
{
	BOOL bRtn = FALSE;
	if( !profile.IsValid() )
	{
	...
	for( int i = 0 ; i < iNum ; i++ )
	{
		...
		if ( !bRtn )
		{
			printf ( "Create Resume Module=%d serialno=%d addr=%d sName=%s stemplate=%s \
...
BOOL CProtocol_Resume::CreateModule( int iModule , int iSerialNo , WORD iAddr , char * sName , char * stplatePath )
{
	CProtocol_Resume * pProtocol = NULL ;

	pProtocol = new Resume;
