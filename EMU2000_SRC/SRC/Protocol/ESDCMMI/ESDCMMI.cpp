...
#define YK_TIME_INTERVAL			6		/*秒,遥控超时时间*/
#define DZ_TIME_INTERVAL			10		/*秒,定值超时时间*/

#define FRAMEBYTE_CONTROLBYTE		0x71	/*子站到主站*/
...
#define FRAMEKIND_DOWN_SETTIME		0x7A	/*主站到子站*/
...
BOOL ESDCMMI::ProcessProtocolBuf( BYTE * buf , int len ) 
{
	...
	while(len>=WORD_LENGTH*2)
	{
		...
		switch(frameKind)
		{
		case FRAMEKIND_DOWN_SETTIME:/*处理设置时钟命令*/
			Rtn = ESDCMMI_setSubstationTime(pData);
			break;
		case FRAMEKIND_DOWN_BECKONTIME:/*处理召唤子站时钟命令*/
			m_bRetTime = TRUE;
			break;
		case FRAMEKIND_DOWN_YKRESET:/*遥控预置*/
			cmdType = YK_SEL;
			Rtn = ESDCMMI_handleYkCmd(pData,cmdType);
			break;
		case FRAMEKIND_DOWN_YKEXECUTE:/*遥控执行*/
			cmdType = YK_EXCT;
			Rtn = ESDCMMI_handleYkCmd(pData,cmdType);
			break;
		case FRAMEKIND_DOWN_YKCANCEL:/*遥控撤销*/
			cmdType = YK_CANCEL;
			Rtn = ESDCMMI_handleYkCmd(pData,cmdType);
			break;
		case FRAMEKIND_DOWN_RESET:/*复归命令*/
			//bReturn=FALSE;
			break;
...
BOOL ESDCMMI::ESDCMMI_assembleYcFrame( BYTE * pBuf , int &len )
{
	...
	while(sendBytes+WORD_LENGTH<=ESDCMMIPACKETLEN && wSendOrder<wYcNum)
	{
		...
		if(wSendOrder>0x7FF || wSendOrder>=wYcNum)/*碰到尾部，避免溢出*/
		{
...
BOOL ESDCMMI::ESDCMMI_assembleYmFrame( BYTE * pBuf , int &len )	
{
	...
	while(sendBytes+WORD_LENGTH<=ESDCMMIPACKETLEN && wSendOrder<wYmNum)
	{
		...
		if(wSendOrder>0x3FF || wSendOrder>=wYmNum)/*碰到尾部，避免溢出*/
		{
...
#if 0					//by cyz 2017-06-27	解决第一条总线为PAUSE时无法发送装置状态问题!
...
#endif
		if( ProtocolType != PROTOCO_GATHER && ProtocolType != PROTOCO_TRANSPROT ){		//by cyz! 2017-06-27 替换上面内容!
			if(byLine == 22){
			curSendType_prior = SEND_YX_FRAME;
			curSendType_normal = SEND_YC_FRAME;
			ESDCMMI_sendOrder = 0;
...
WORD ESDCMMI::ESDCMMI_addPriorWord( BYTE *pBuf , WORD nByte )
{
	...
	if ((m_bRetTime == TRUE)&&(bytes+WORD_LENGTH*2 <= nByte)) /*是否主站要求要求子站返送时间*/
	{
...
BYTE ESDCMMI::ESDCMMI_assembleYkRevise(BYTE* pBuf)
{
	...
	if(ESDCMMI_Yk_Data.m_byYkCmd == YK_SEL_RTN)/*处理遥控预置*/
	{
		...
		{
			if(ESDCMMI_Yk_Data.m_byStatus == YK_PROCESS_RECEIVE)/*预置成功*/
			{
				ESDCMMI_Yk_Data.m_byValid=YK_INVALID;
				ESDCMMI_Yk_Data.m_byStatus=YK_PROCESS_SUCCESS;/*插入三次*/
			...
			if(intSec > YK_TIME_INTERVAL)/*预置超时*/
			{
				ESDCMMI_Yk_Data.m_byValid=YK_INVALID;
				ESDCMMI_Yk_Data.m_byStatus=YK_PROCESS_OVERTIME;/*插入三次*/
				ESDCMMI_InsertWord(pBuf0,0xE1,YK_TYPE_NORMAL,YK_PROTOCOL_ERR,yk_lineNo,module_addrNo,module_ykNo);
				pBuf0 += WORD_LENGTH;
				memcpy(pBuf0,pBuf0-WORD_LENGTH,WORD_LENGTH);
				pBuf0 += WORD_LENGTH;
...
BOOL ESDCMMI::ESDCMMI_GetYxWordFromYxChP(WORD YxOrder,BYTE* pBuf)
{
	...
	baseYxOrder=funcCode*32;/*每个遥信字遥从以32为度量的基数开始*/
...
BOOL ESDCMMI::ESDCMMI_insertYxChp(WORD YxChp,BYTE* pBuf)
{
	...
	memcpy(pBuf,yxWord,WORD_LENGTH);/*连续插入遥信字3遍*/
	pBuf += WORD_LENGTH;
...
BOOL ESDCMMI::ESDCMMI_setSubstationTime(BYTE* pBuf)
{
	...
	if((pointer[0]!=0)||(pointer[1]!=0xEE))/*协议规定*/
		return FALSE;
	...
	if((pointer[0]!=0)||(pointer[1]!=0xEF))/*协议规定*/
		return FALSE;
	hour = pointer[2];
	day	= pointer[3];
	month	= pointer[4];
...
BOOL  ESDCMMI::ESDCMMI_handleYkCmd(BYTE* pBuf,BYTE cmdType)
{
	...
	time1 = time(NULL);
	intSec = (int)difftime(time1,time0);
	if(abs(intSec) > YK_TIME_INTERVAL)/*预置超时*/
	{
		ESDCMMI_Yk_Data.m_byValid=YK_INVALID;
	...
	case YK_PROTOCOL_CANCEL:
		module_action=ESDCMMI_Yk_Data.m_byYkAction;/*执行、撤销*/
		break;
	default:
		return FALSE;/*其它动作码暂不支持*/
	}
	...
	if(YK_SEL!=cmdType)/*遥控执行、遥控撤消都不反校*/
	{
		ESDCMMI_Yk_Data.m_byValid=YK_INVALID;
	}
...
BOOL ESDCMMI::WritePIVal(WORD wSerialNo ,WORD wPnt, DWORD dwVal)
{
