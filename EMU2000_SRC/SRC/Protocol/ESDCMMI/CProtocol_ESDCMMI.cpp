...
CProtocol_ESDCMMI::CProtocol_ESDCMMI()
{
...
CProtocol_ESDCMMI::~CProtocol_ESDCMMI()
{
	...
	m_module.clear() ;
	printf( "Delete All CProtocol_ESDCMMI OK . \n" );
}

BOOL CProtocol_ESDCMMI::Init( BYTE byLineNo )
{
	//通过总线号获取读取的装置文件路径
	m_byLineNo = byLineNo ;
	//读取模板文件
...
BOOL CProtocol_ESDCMMI::GetDevData( )
{
	memset( m_sDevPath , 0 , sizeof( m_sDevPath ) ) ;
	sprintf( m_sDevPath , "%s/ESDCMMI/%s%02d.ini" , SYSDATAPATH , DEVNAME , m_byLineNo + 1 );
	CProfile profile( m_sDevPath ) ;
	return ProcessFileData( profile ) ;
}

BOOL CProtocol_ESDCMMI::ProcessFileData( CProfile &profile )
{
	BOOL bRtn = FALSE;
	if( !profile.IsValid() )
	{
	...
	for( int i = 0 ; i < iNum ; i++ )
	{
		...
		if ( !bRtn )
		{
			printf ( "Create ESDCMMI Module=%d serialno=%d addr=%d sName=%s stemplate=%s \
...
BOOL CProtocol_ESDCMMI::CreateModule( int iModule , int iSerialNo , WORD iAddr , char * sName , char * stplatePath )
{
	CProtocol_ESDCMMI * pProtocol = NULL ;

	pProtocol = new ESDCMMI;
...
BYTE CProtocol_ESDCMMI::ESDCMMI_GetCRC(BYTE  *buf, int len)
{
	BYTE CRC = 0,index=0;
	BYTE * pStart = buf;
