// Protocol_UPS.cpp: implementation of the CProtocol_UPS class.
//
//////////////////////////////////////////////////////////////////////
...
CProtocol_UPS::~CProtocol_UPS()
{
	...
	m_module.clear() ;
	printf( "Delete All CProtocol_UPS OK . \n" );
}



BOOL CProtocol_UPS::GetProtocolBuf( BYTE * buf , int &len , PBUSMSG pBusMsg ) 
{
	return TRUE;
}

BOOL CProtocol_UPS::ProcessProtocolBuf( BYTE * buf , int len ) 
{
	return FALSE ;
}

BOOL CProtocol_UPS::Init( BYTE byLineNo )
{
	//增加UPS 采集模块数据
	//通过总线号获取读取的装置文件路径
	m_byLineNo = byLineNo ;
...
BOOL CProtocol_UPS::ProcessFileData( CProfile &profile )
{
	BOOL bRtn = FALSE;
	if( !profile.IsValid() )
	{
...
BOOL CProtocol_UPS::CreateModule( int iModule , int iSerialNo , WORD iAddr , char * sName , char * stplatePath )
{
	CProtocol_UPS * pProtocol = NULL ;
