...
typedef struct{			//没必要指定个数!
	BYTE type;			//预留
	UINT yc_func[4];	//四个字节128位，表示128个功能码!		//用于过滤指定的功能码!
	WORD yx_func;		//遥信16个功能码!
	UINT yx_bitmap;		//用于指定取哪些位!
	UINT ym_func[2];	//遥脉64个功能码!
	BYTE switchorder;	//开关顺序!默认0
	BYTE srcaddr;		//默认1						//源地址与目的地址都已经通过PBUSMSG结构体指定了，该成员就没有必要存在于此了吧!	答曰:PBUSMSG结构体中的地址应该既含有设备地址又含有总线号。而cdt协议只有一个字节指定数据那么该地址是指的什么暂时尚不清楚!
	BYTE dstaddr;		//默认1						//源地址和目的地址，应该从上层获得。
}ConfigItem;
