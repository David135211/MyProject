...
#define MBS_BUS_RSADDR	 50000  //总线接收发送帧数寄存器地址
...
#define SETYM			0x0004
#define SETYK			 0x0008
#define SET_DEVSTATE_YX	 0x0010 //设置装置状态遥信
#define SET_BUS_COUNT	0x0020  //设置装置通讯接收/发送计数
    ...
    case YM:
        {
			...
			DWORD dwVal = 0 ;
			if( wStn >0 && wPnt >0 )
				m_pMethod->GetYmData( wPntNum - 1 , wPntNum - 1 , dwVal ) ;
			else
				return FALSE ;

			memcpy(v, &dwVal , sizeof(DWORD));
		}
        break;
    default:
        return FALSE ;
...
{/*{{{*/
	...
	BYTE * pt = &pBuf[0];
	WORD crc = GetCRC( pt, len-2);/*计算校验码*/
	if( !( LOBYTE(crc) == *(pt+len-2) && HIBYTE(crc) == *(pt+len-1)) )/*校验码判断是否正确*/
		return FALSE ;
...
{/*{{{*/
	...
	else if(wRegNum<=0 || wRegNum>16 || ( wRegNum % 2 != 0 ) )
	{
		SetErrorMsg( byFunc , ERR_DATA ) ;
		return FALSE ;
	}
