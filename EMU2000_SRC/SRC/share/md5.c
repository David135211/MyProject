...
static void MD5_Update(MD5Context * context, unsigned char * buf, int len)
{
...
static void MD5_Final(MD5Context * context, unsigned char digest[16])
{
...
unsigned char * GetFileMD5 (char * filename , unsigned char *pchMD5FileBuf)
{
    ...
    else
    {
       ...
       MD5_Final(&context,pchMD5FileBuf);
       fclose (file);
	   // for(i=0;i<16;i++) 
	   // { 
		   // printf("%x",(chMD5FileBuf[i] & 0xF0)>>4); 
		   // printf("%x",chMD5FileBuf[i] & 0x0F); 
	   // } 
	   // printf("\n"); 
	}

	return pchMD5FileBuf;
}
...
void MD5( char * md5 , char * sequence )
{
	...
	unsigned char buff[16];
 	...
 	len = strlen(sequence);
   	MD5_Init(&context);
	MD5_Update(&context,(unsigned char *)sequence, len);
   	MD5_Final(&context,buff);
   	for(j=0;j<16;j++)
   	{
   		//printf("%x",(buff[j] & 0xF0)>>4);
       	//printf("%x",buff[j] & 0x0F);
		sprintf (md5+2*j,"%02x", buff[j]);
...
/*
 *--------------------------------------------------------------------------------------
 *      Method:  IsFileMD5Right
 * Description:  MD5 是否和文件的md5相匹配
 *       Input:  文件名
 *				 md5缓冲
 *		Return:
 *--------------------------------------------------------------------------------------
...
bool IsFileMD5Right ( char *pchFileName, char *pchMd5Buf )
{
	...
	if( NULL != GetFileMD5( pchFileName, (unsigned char *)chMd5Buf ) )
	{
		if( 0 == strncmp( chMd5Buf, pchMd5Buf, 16 ) )	
		{
			return true;
		}
	}
	...
	return false;
}		/* -----  end of method CSocketFtpFile::IsFileMD5Right  ----- */
